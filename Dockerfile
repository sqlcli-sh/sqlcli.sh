...
RUN python -m venv /opt/sqlcli-env
ENV PATH /opt/sqlcli-env/bin:$PATH
ENV VIRTUAL_ENV /opt/sqlcli-env

# Set the working directory.
WORKDIR /workspaces/sqlcli/

# Install the run time Python dependencies in the virtual environment.
COPY --chown=user:user poetry.lock* pyproject.toml /workspaces/sqlcli/
RUN mkdir -p /home/user/.cache/pypoetry/ && mkdir -p /home/user/.config/pypoetry/ && \
    mkdir -p src/sqlcli/ && touch src/sqlcli/__init__.py && touch README.md
...
COPY --chown=user:user .pre-commit-config.yaml /workspaces/sqlcli/
...
ENTRYPOINT ["/opt/sqlcli-env/bin/sqlcli"]
CMD []
